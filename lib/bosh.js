...
function BOSHConnection(options) {
    var self = this
    EventEmitter.call(this)

    this.boshURL = options.bosh.url
    ...
    if (options.xmlns) {
        var key
        for (var prefix in options.xmlns) {
            if (prefix) {
                key = 'xmlns:' + prefix
                this.xmlnsAttrs[key] = options.xmlns[prefix]
            } else {
                this.xmlnsAttrs.xmlns = options.xmlns[prefix]
            }
        }
    }
    this.currentRequests = 0
    this.queue = []
    this.rid = options.rid || Math.ceil(Math.random() * 9999999999)
    ...
    if (options.bosh.sid) {
        requestOptions.sid = this.sid = options.bosh.sid
    }
    this.request(requestOptions,
        [],
        function(error, bodyEl) {
            if (error) {
                self.emit('error', error)
            } else if (bodyEl && bodyEl.attrs) {
                self.sid = bodyEl.attrs.sid
                self.maxRequests = parseInt(bodyEl.attrs.requests, 10) || 2
                if (self.sid && (self.maxRequests > 0)) {
                    self.emit('connect')
                    self.processResponse(bodyEl)
                    process.nextTick(self.mayRequest.bind(self))
                } else {
                    self.emit('error', 'Invalid parameters')
                }
            }
        })
}
...
BOSHConnection.prototype.mayRequest = function() {
    ...
    this.queue = []
    this.rid++
    this.emit('bosh.parameters', { rid: this.rid, sid: this.sid })
